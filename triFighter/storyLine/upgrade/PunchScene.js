...
class PunchScene extends Phaser.Scene {
  ...
  create() {
    ...
    const textBoxMap = this.make.tilemap({
      data: textBoxData,
      tileWidth: 32,
      tileHeight: 32,
    });
    const textBoxCoords = [100, 200];
    const textBox = textBoxMap
      .createLayer(0, triComTiles, textBoxCoords[0], textBoxCoords[1])
      .setScale(2);
    const style = {};
    const upgradeText = this.add.text(
      textBoxCoords[0] + 5,
      textBoxCoords[1] + 5,
      "Our base is stronger!",
      style
    );
